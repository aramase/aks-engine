...
func (a *Properties) validateMasterProfile(isUpdate bool) error {
	...
	if a.OrchestratorProfile.OrchestratorType == Kubernetes {
		...
		if a.FeatureFlags.IsIPv6DualStackEnabled() || a.FeatureFlags.IsIPv6OnlyEnabled() {
			if m.Distro == CoreOS {
				return errors.Errorf("Dual stack and IPv6 only feature is currently supported only with Ubuntu, but master is of distro type %s", m.Distro)
			}
		}
	}
	...
	for i, agentPoolProfile := range a.AgentPoolProfiles {
		...
		if a.FeatureFlags.IsIPv6DualStackEnabled() || a.FeatureFlags.IsIPv6OnlyEnabled() {
			if agentPoolProfile.OSType == Windows {
				return errors.Errorf("Dual stack and IPv6 only feature is supported only with Linux, but agent pool '%s' is of os type %s", agentPoolProfile.Name, agentPoolProfile.OSType)
			}
			if agentPoolProfile.Distro == CoreOS {
				return errors.Errorf("Dual stack and IPv6 only feature is currently supported only with Ubuntu, but agent pool '%s' is of distro type %s", agentPoolProfile.Name, agentPoolProfile.Distro)
			}
		}
